...
public class BFSearch {
	...
	// Breadth-first search algorithm, start is the sender node, step is the degree of neighbors
	public void execute(Graph graph, Vertex start, int step) {
		...
		while (!unSettledNodes.isEmpty()) {
			Vertex current = unSettledNodes.remove(0); // FIFO
			...
			for (Vertex target : neighbors) {
				// if the vertex has been visited, either settled or in the waiting list, continue.
				if (isSettled(target) || isUnsettled(target)) continue; 
				// for unvisited vertex, update its distance to the source.
